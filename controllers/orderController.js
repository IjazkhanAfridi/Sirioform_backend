...
const PatentUpdate = require('../models/PatentUpdate'); // Add this import
...
const createOrderController = async (req, res) => {
  const { productIds, quantities, fromCart, note, productType = 'kit' } = req.body; // Add productType
  ...
  try {
    ...
    // Only validate quantity for kits, not for patent updates
    if (productType === 'kit') {
    ...
    // Only get progressive numbers for kits
    if (productType === 'kit') {
      const orders = await Order.find();
      orders.forEach((order) => {
        order.orderItems.forEach((item) => {
          if (item.progressiveNumbers && item.progressiveNumbers.length > 0) {
    ...
    for (let i = 0; i < productIds.length; i++) {
      ...
      if (productType === 'patent') {
        // Handle patent updates
        product = await PatentUpdate.findById(productIds[i]);
        if (!product) {
          return res
            .status(404)
            .json({ message: `Patent update with id ${productIds[i]} not found` });
        }
        
        price = product.cost; // Patent updates have fixed cost
        
        // Add to orderItems (no progressive numbers for patent updates)
        orderItems.push({
          productId: productIds[i],
          quantity: quantities[i] || 1,
          totalQuantity: quantities[i] || 1,
          price: price,
          progressiveNumbers: [], // Empty for patent updates
        });
        
        // Add to product info for notification
        orderProductInfo.push({
          quantity: quantities[i] || 1,
          type: product.code, // Use code for patent updates
        });
        
        totalPrice += price * (quantities[i] || 1);
        
      } else {
        // Handle kits (existing logic)
        product = await Kit.findById(productIds[i]);
        if (!product) {
          return res
            .status(404)
            .json({ message: `Kit with id ${productIds[i]} not found` });
        }
        ...
        // Generate progressive numbers for kits
        ...
        orderProductInfo.push({
          quantity: quantities[i],
          type: product.type, // Use type for kits
        });
    ...
    // Handle cart cleanup if order was from cart
    if (fromCart) {
    ...
    if (req.user?.role == 'admin') {
      const orders = await Order.find()
        .populate(
          ...
          'firstName lastName role name type code isRefreshCourse isForInstructor isForTrainer cost'
        )
        .populate('orderItems');
      res.json(orders);
    } else {
...
const getProdottiAcquistati = async (req, res) => {
  try {
      ...
      order.orderItems.forEach((item) => {
        ...
        if (prodotto) {
          ...
          acc.push({
            ...
            title: item.productId.type || item.productId.code,
            quantity: item.quantity,
            isRefreshKit: item.productId.isRefreshKit || false,
            isForInstructor: item.productId.isForInstructor || false,
            isForTrainer: item.productId.isForTrainer || false,
    ...
    // Calculate assigned and used quantities (only for kits with progressive numbers)
    prodottiAcquistati?.forEach((prodotto) => {
      courses?.forEach((course) => {
        if (prodotto.progressiveNumbers.length > 0) {
