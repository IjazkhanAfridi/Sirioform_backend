...
} = require('./utils/courseExpirationService');
...
const testExpirationFunctionality = async () => {
  try {
    ...
    if (sampleDiscente) {
      console.log(
        `Found discente: ${sampleDiscente.nome} ${sampleDiscente.cognome}`
      );
      ...
      if (sampleDiscente.kitAssignments.length > 0) {
        ...
        if (!firstAssignment.courseCompletedDate) {
          ...
          firstAssignment.expirationDate = new Date(
        ...
        const testCourse = await Course.findOne({
          tipologia: firstAssignment.courseId?.tipologia,
        }).populate('tipologia');
        if (testCourse) {
          const eligibility = await canAssignDiscenteToCourse(
            sampleDiscente._id.toString(),
            testCourse.tipologia._id.toString()
          );
    ...
    if (sampleInstructor) {
      console.log(
        `Found instructor: ${sampleInstructor.firstName} ${sampleInstructor.lastName}`
      );
      ...
      if (sampleKit) {
        ...
        const eligibility = await canAssignInstructorToKitType(
          sampleInstructor._id.toString(),
          sampleKit._id.toString()
        );
    ...
    if (sampleDiscente && sampleDiscente.kitAssignments.length > 0) {
      ...
      if (assignment.courseId) {
        const result = await markDiscenteCourseCompleted(
          sampleDiscente._id.toString(),
    ...
    if (sampleInstructor) {
      const sampleCourse = await Course.findOne({
        istruttore: sampleInstructor._id,
      });
      if (sampleCourse) {
        const result = await markInstructorCourseCompleted(
          sampleInstructor._id.toString(),
          sampleCourse._id.toString()
        );
